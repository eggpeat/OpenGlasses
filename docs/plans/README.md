...
| [AM](embedding-quality-upgrade.md) | Embedding Quality Upgrade | 🚧 In progress — substrate shipping: `EmbeddingVersion` + migration policy + `DocumentStore` stamp/self-heal (re-embed on model change). Next: `NLContextualEmbedding` (transformer, multilingual) behind the `Embedder` seam + route `SemanticMemoryStore`; sharpens RAG, memory, and skill/memory retrieval |
