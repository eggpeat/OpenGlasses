...
| [BH](BH-gateway-remote-invoke.md) | Gateway Remote Invoke | 📋 Planned — agent-initiated glasses control over the existing gateway socket: pure `RemoteCommandParser`/`RemoteCommandPolicy`/reply envelope (deny-by-default, Agent Mode-gated, capture class behind BC gate, never silent, audited) + thin `RemoteCommandExecutor` onto existing services; folds in WS token redaction, reconnect jitter/give-up, degenerate-frame guard, paste-friendly secret fields. Shares the BG P2 command taxonomy |
| [BI](BI-uncertainty-web-search.md) | Uncertainty-Triggered Web Search (local backends) | 📋 Planned — local-only gate: pure `UncertaintyDetector` (answer-hedging + question-freshness signals) + `UncertaintyReask` (single re-ask grounded in `WebSearchTool` results) wired into the MLX and Apple Foundation completion paths. Flag-gated, transparent (never silent), foreground-only. Fixes confident-but-stale local answers to time-sensitive questions where cloud backends would tool-call |
