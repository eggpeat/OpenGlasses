...
| [Study Mode (flashcards + quizzes)](study-mode.md) | 🚧 PR 1 core shipped on `feat/study-mode` — active-recall study on Document RAG/OCR. Deterministic core: `StudyModels` + pure `QuizGrader` + Leitner `SpacedRepetition` + `StudyContentBuilder` (prompt/schema/parse-validate) + `StudyStore`; 15 tests, Debug green. Deferred: `StudyService` + `study` tool + review flow + SwiftUI views + scan source + voice-answer matcher. |
